local vars = {
	...
	hybrid_firstperson_mouse2_snap = CreateConVar("cl_nte_hybrid_mouse2_snap", "0", FCVAR_ARCHIVE, "Enable/disable old Mouse2 snapping behavior in hybrid FP."),
...
local function main(ply, pos, angles, fov, znear, zfar)
    ...
    if vars.mode:GetInt() == FP then
        ...
        -- Check if hybrid_firstperson is enabled AND Mouse2 is down AND the new snap ConVar is enabled
        if vars.hybrid_firstperson:GetBool() and lp:KeyDown(IN_ATTACK2) and vars.hybrid_firstperson_mouse2_snap:GetBool() then
...
local function preferences(Panel)
	...
	Panel:CheckBox("Enable old Mouse2 snap in Hybrid FP", vars.hybrid_firstperson_mouse2_snap:GetName())
    Panel:ControlHelp("Enables the original broken Mouse2 camera snapping behavior in Hybrid Firstperson mode.")
	Panel:ControlHelp("Keep off unless you know what you're doing!")
	Panel:ControlHelp("")
